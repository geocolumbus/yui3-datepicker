...
Y.DatePicker.prototype.setDate = function (newDate, discardOldTime) {
    if (!discardOldTime) {
        var oldDate = this.parseDate(this.input.get("value"));
        if (oldDate) {
    ...
    var str = Y.DataType.Date.format(newDate, {format: this.format});
...
Y.DatePicker.prototype.setCustomRenderer = function () {
    if (this.customRenderer)
        return;
    var self = this;
    var canBeSelected = function (date) {
    ...
    var filterFunction = function (date, node) {
    ...
    var f = this.calendar._canBeSelected;
...
Y.DatePicker.prototype.showContainer = function (e) {
    ...
    if (this.onshow)
        this.onshow();
    var x = this.calendarLauncher.getX();
    var y = this.calendarLauncher.getY() +
            parseInt(this.calendarLauncher.getComputedStyle("height"));
    this.container.setStyles({left: x, top: y});
    var date = this.parseDate(this.input.get("value"));
    if (date) {
